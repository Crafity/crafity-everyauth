...
var oauth2 = module.exports =
  everyModule.submodule('oauth2')
    ...
    .configurable({
      apiHost: 'e.g., https://graph.facebook.com', oauthHost: 'the host for the OAuth provider', appId: 'the OAuth app id provided by the host', appSecret: 'the OAuth secret provided by the host', authPath: "the path on the OAuth provider's domain where " +
        "we direct the user for authentication, e.g., /oauth/authorize", accessTokenPath: "the path on the OAuth provider's domain " +
        "where we request the access token, e.g., /oauth/access_token", accessTokenHttpMethod: 'the http method ("get" or "post") with which to make our access token request', postAccessTokenParamsVia: '"query" to POST the params to the access ' +
        'token endpoint as a querysting; "data" to POST the params to ' +
        'the access token endpoint in the request body', myHostname: 'e.g., http://local.host:3000 . Notice no trailing slash', alwaysDetectHostname: 'does not cache myHostname once. Instead, re-detect it on every request. Good for multiple subdomain architectures', redirectPath: 'Where to redirect to after a failed or successful OAuth authorization', convertErr: 'a function (data) that extracts an error message from data arg, where `data` is what is returned from a failed OAuth request', authCallbackDidErr: 'Define the condition for the auth module determining if the auth callback url denotes a failure. Returns true/false.'
    })
    ...
    //.optional()
    .accepts('session accessToken extra oauthUser req')
    .promises('user')
    .step('compile')
    ...
    .getAuthUri(function (req, res) {
      ...
      var params = {
          client_id: this.appId(), redirect_uri: this.myHostname() + this.callbackPath()
        }
    ...
    .getAccessToken(function (code) {
      var p = this.Promise()
        , params = {
          client_id: this.appId(), redirect_uri: this.myHostname() + this.callbackPath(), code: code, client_secret: this.appSecret()
        }
    ...
    .compile(function (accessToken, extra, oauthUser, user) {
      var compiled = {
        accessToken: accessToken, oauthUser: oauthUser, user: user
      };

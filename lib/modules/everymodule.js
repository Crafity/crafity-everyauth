...
var everyModule = module.exports = {
  name: 'everymodule', definit: function (fn) {
    ...
    return this;
  }, get: route('get'), post: route('post'), stepseq: function (name, description) {
    ...
    //      readability
    return this;
  }, cloneOnSubmodule: ['cloneOnSubmodule', '_configurable'], submodules: {}
...
everyModule
  .configurable({
    moduleTimeout: 'how long to wait per step ' +
      ...
      'timeout callbacks', moduleErrback: 'THE error callback that is invoked' +
      'any time an error occurs in the module; defaults to `throw` wrapper', logoutRedirectPath: 'where to redirect the app upon logging out', findUserById: 'function for fetching a user by his/her id -- used to assign to `req.user` - function (userId, callback) where function callback (err, user)'
  })
  .get('logoutPath')
  .step('handleLogout')
  .accepts('req res')

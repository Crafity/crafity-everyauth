...
var openid = module.exports =
  everyModule.submodule('openid')
    .configurable({
      simpleRegistration: 'e.g., {"nickname" : true}', attributeExchange: 'eg {"http://axschema.org/contact/email": "required"}', myHostname: 'e.g., http://localhost:3000 . Notice no trailing slash', alwaysDetectHostname: 'does not cache myHostname once. Instead, re-detect it on every request. Good for multiple subdomain architectures', redirectPath: 'The path to redirect To', openidURLField: 'The post field to use for open id'
    })
    .definit(function () {
    ...
    .accepts('session userAttributes req')
    .promises('user')
    .step('addToSession')
    ...
    .simpleRegistration({
      "nickname": true, "email": true, "fullname": true, "dob": true, "gender": true, "postcode": true, "country": true, "language": true, "timezone": true
    })
    .attributeExchange({
      "http://axschema.org/contact/email": "required", "http://axschema.org/namePerson/friendly": "required", "http://axschema.org/namePerson": "required", "http://axschema.org/namePerson/first": "required", "http://axschema.org/contact/country/home": "required", "http://axschema.org/media/image/default": "required", "http://axschema.org/x/media/signature": "required"
    })

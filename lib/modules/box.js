...
var box = module.exports =
  everyModule.submodule('box')
    .configurable({
      apiKey: 'The API key obtained when registering a project with OpenBox', redirectPath: 'Where to redirect to after a failed or successful auth'
    })
    ...
    //.optional()
    .accepts('session authToken boxUser req')
    .promises('user')
    .step('compileAuth')
    ...
    .getTicket(function (req, res) {
      ...
      rest.get(this._apiHost + '/rest', {
        parser: rest.parsers.xml, query: {
          action: 'get_ticket', api_key: this._apiKey
        }
    ...
    .fetchUser(function (authToken) {
      ...
      rest.get(this._apiHost + '/rest', {
        parser: rest.parsers.xml, query: {
          action: 'get_account_info', api_key: this._apiKey, auth_token: authToken
        }
      }).on('success',function (data, res) {
          var status = data.status
            , user = data.user;
    ...
    .compileAuth(function (authToken, boxUser, user) {
      return compiled = {
        authToken: authToken, boxUser: boxUser, user: user
      };
    })

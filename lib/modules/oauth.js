...
var oauth = module.exports =
  everyModule.submodule('oauth')
    ...
    .accepts('session accessToken accessTokenSecret oauthUser req')
    .promises('user')
    .step('compileAuth')
    ...
    .getRequestToken(function (req, res) {
      ...
      this.oauth.getOAuthRequestToken({ oauth_callback: this._myHostname + this._callbackPath }, function (err, token, tokenSecret, params) {
        if (err && (!err.data || !~(err.data.indexOf('Invalid / expired Token')))) {
          return p.fail(err);
        }
    ...
    .compileAuth(function (accessToken, accessTokenSecret, oauthUser, user) {
      return {
        accessToken: accessToken, accessTokenSecret: accessTokenSecret, oauthUser: oauthUser, user: user
      };
    })
